...
import java.util.List;
    ...
    private final CompraMapper compraMapper;
            ...
            // --- CÁLCULO CORRECTO DEL PRECIO CON DESCUENTO ---
            BigDecimal precioEfectivo = producto.getPrecio();
            try {
                BigDecimal porcentajeDescuento = ofertaService.obtenerDescuentoActivo(producto.getId());
                if (porcentajeDescuento != null && porcentajeDescuento.compareTo(BigDecimal.ZERO) > 0) {
                    BigDecimal descuentoAplicar = producto.getPrecio()
                            .multiply(porcentajeDescuento)
                            .divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);

                    precioEfectivo = producto.getPrecio().subtract(descuentoAplicar);
                }
            } catch (Exception e) {
                // Si ocurre algún contratiempo, se mantiene el precio base por seguridad
            }

            BigDecimal subTotal = precioEfectivo.multiply(BigDecimal.valueOf(item.getCantidad()));
